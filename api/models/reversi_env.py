...
class ReversiEnv(gym.Env):
    ...
    def step(self, action, color=0):
        ...
        if ReversiEnv.pass_place(self.board_size, action):
        ...
        elif not ReversiEnv.valid_place(self.state, action, color):
            ...
            ReversiEnv.make_place(self.state, action, color)

        self.possible_actions = ReversiEnv.get_possible_actions(self.state, color)
        reward = ReversiEnv.game_finished(self.state)
        if color == ReversiEnv.WHITE:
            reward = - reward
        self.done = reward != 0
